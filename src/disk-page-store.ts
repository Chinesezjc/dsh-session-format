 ...
 * renamed over the target, and the directory is fsynced. Sync counterpart of
 * `writeFileAtomicDurable` in `./atomic-write.ts`, kept here so the page store
 * can serve the synchronous `PageStore` surface.
 * @param filename - final path receiving the content.
 * @param content - complete next file content.
 * @param mode - permission bits for the replacement inode.
 */
function writeFileAtomicDurableSync(filename: string, content: Uint8Array, mode: number): void {
