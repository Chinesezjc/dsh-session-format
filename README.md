...
- **In-memory prototype only** — the in-memory B+Tree is not yet backed by a single durable multi-page file format; `DiskPageStore` persists all pages in one append-only segment file instead.
