...
- **Append allocates in O(1) but commits the whole snapshot** — `append` mints the EventId/BlobId from the persisted high-water marks (`nextEventCounter`, `blobIdWatermark`), which `createSession` calibrates past every id visible in the file at registration and the commit point enforces (`validateSessionFile` rejects a record whose counter or watermark sits below its own ids), so the per-call id scans are gone. The commit checks blob immutability against the in-memory previous generation passed by the repository and reads retained backup pages lazily only for ids the previous map no longer carries, so the per-commit backup re-reads are gone too. The commit still rewrites the whole snapshot (multi-page tree pages, the full blob-map page, and the serialize→deserialize validation round-trip), so appending to an unbounded session stays O(n) per call from the snapshot write; incremental page persistence is deferred.
